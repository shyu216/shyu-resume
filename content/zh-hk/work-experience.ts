...
export const workExperience: WorkExperience[] = [
	{
		...
		bullets: [
			"帶領3人敏捷團隊在混合辦公環境下工作；使用**Next.js SSR PWA**和**AWS**雲端基礎設施交付80+生產功能，涵蓋消費者/餐廳應用，實現100% UAT通過率",
			"構建雲端原生全棧容器租賃平台，包含**Next.js**前端、**AWS DynamoDB**後端和**RESTful API**集成，實現獨特餐具ID和借還追蹤，簡化員工從QR掃描到餐具追蹤的工作流程",
			...
			"使用**AWS CDK/Amplify**設計**CI/CD**流水線和基礎設施即代碼（**IaC**），實現分支隔離和環境一致性，將部署時間從數小時縮短至5分鐘，消除手動配置混亂",
			"管理多環境基礎設施，使用**Cloudflare** DNS/路由覆蓋開發/測試/生產環境；實現**i18n**國際化，支持英文/繁體中文切換，使用可複用**React Hooks**",
			"構建事件驅動通知系統，使用**AWS EventBridge/SES**和可定制HTML模板，自動化借還提醒和支付通知",
			"製作並部署應用程式內教程影片，降低新用戶和餐廳員工的上手門檻",
			"透過Asana/WhatsApp與CEO和業務持份者協作，進行需求收集、衝刺規劃和交付時間線管理；為其他開發者提供專案結構和包設置建議"
		]
	}
];
