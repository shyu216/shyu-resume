...
export const projects: Project[] = [
  ...
  },
  {
    ...
    bullets: [
      "IERG4300（大数据分析）：基于 Hadoop/MapReduce 设计分布式算法系统，实现并行 Dijkstra、PageRank、K-Means，掌握大数据处理流程优化技巧",
      "CSCI3170（数据库）：设计 5 张带主键/外键的关联表，编写 8 个高复杂度 SQL 查询（多表联查/嵌套子查询/自定义公式），完成联赛/球队/赞助商的体育数据统计分析",
      "GEOM90008（空间数据库）：基于 PostGIS 实现空间 SQL 查询，落地地理距离计算、路径规划、高程分析等 GIS 场景，覆盖单点高程查询、区域坡度统计等核心需求",
      "COMP90054（AI 规划）：针对 Splendor 游戏设计多策略 AI 智能体，抽象 MDP 状态空间/设计奖励函数，优化规划 Agent（击败 TA 基准，27 胜 13 负），掌握搜索/调参",
    ],
    rank: 1,
  },
];
