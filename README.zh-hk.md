...
想換顏色和字體？小事一樁！來看看這些檔案：

| 檔案                                    | 做什麼的                     |
| ------------------------------------- | ------------------------ |
| `lib/theme-config.ts`                 | 顏色配置和字體設定 —— 視覺效果的精髓都在這啦 |
...
2. 在 `components/job/job-stack-keywords.ts` 更新關鍵詞映射

#### 4️⃣ AI 驅動的關鍵詞生成（超厲害！🤖）

我們有個 AI 加持的關鍵詞匹配系統！神奇的魔法配方：

1. 打包你的履歷內容（工作經驗 + 專案）
2. 發送給 AI 小助手，比如豆包
3. 客氣地請它生成一個完整的 `keywords.json` 檔案
4. 用生成的檔案替換現有的 `app/keywords.json`

哇塞！你的履歷現在超級適合各種職位類型，ATS 相容性也 up up！🚀

你也可以修改 `scripts/gen-keywords.js` 檔案來定製你的關鍵詞生成過程。這個腳本會被 workflow 調用，但具體實現細節由你做主！發揮你的才智，實現你自己的關鍵詞生成邏輯吧！
